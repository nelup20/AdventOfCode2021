...
def find_winning_board(numbers, boards)
  ...
  (0..numbers.length - 1).each do |i|
    (0..boards.length - 1).each do |j|
      next if winning_boards.include?(j)

      mark_board(boards[j], numbers[i])

      next unless bingo?(boards[j])

      winning_boards.push(j)
      winning_numbers.push(i)
    end
  end

  [boards[winning_boards[-1]], numbers[winning_numbers[-1]]]
end
